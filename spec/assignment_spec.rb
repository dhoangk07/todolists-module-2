...
describe "Assignment" do
    ...
    context 'rq07' do
      it "Created the four databases (Users, Profiles, TodoLists and TodoItems)" do
        expect(User.table_exists?).to be true
        expect(Profile.table_exists?).to be true
        expect(TodoList.table_exists?).to be true
        expect(TodoItem.table_exists?).to be true
      end

      it "TodoList contains a reference to User" do
        expect(TodoList.column_names.include? "user_id").to be true
      end
    end
